"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.replaceClientCreation = void 0;
const utils_1 = require("../utils");
// Replace v2 client creation with v3 client creation.
const replaceClientCreation = (j, source, { v2ClientName, v2ClientLocalName, v2GlobalName }) => {
    if (!v2GlobalName)
        return;
    source
        .find(j.NewExpression, (0, utils_1.getClientNewExpression)({ v2GlobalName, v2ClientName }))
        .replaceWith((v2ClientNewExpression) => j.newExpression(j.identifier(v2ClientLocalName), v2ClientNewExpression.node.arguments));
};
exports.replaceClientCreation = replaceClientCreation;
