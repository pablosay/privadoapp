        ...
        try {
            ...
            try {
                ...
                if(await bcrypt.compare(currentPassword, hash)){
                    ...
                    const updateQuery = `UPDATE device SET password = $1 WHERE model = $2`
                    
                    const updateData = [newHash, device]
                    
                    try {
                        
                        await db_connection.query(updateQuery, updateData)
                        
                        res.json({"message": "Password updated"})
                        
                    } catch (errorUpdate) {
                        
                        res.json({"message": "Error while updating password"})
